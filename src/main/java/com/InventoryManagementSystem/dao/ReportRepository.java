...
//                                                                   @Param("brand") String brand);
    ...
    @Query(value = "SELECT ptl.transaction_id, ptl.product_id, ptl.created_at, ptl.quantity_changed, " +
            "ptl.transaction_type, p.barcode, p.brand, p.price_per_unit, p.product_name, p.quantity, " +
            "p.updated_at, p.category_id, v.vendor_name, c.customer_name " +
            "FROM product_transaction_log ptl " +
            "INNER JOIN products p ON ptl.product_id = p.product_id " +
            "LEFT JOIN vendor v ON ptl.vendor_or_customer_id = v.vendor_id AND ptl.transaction_type = 'added' " +
            "LEFT JOIN customers c ON ptl.vendor_or_customer_id = c.customer_id AND ptl.transaction_type = 'removed' " +
            "WHERE p.product_name = :productName AND p.brand = :brand",
            nativeQuery = true)
    List<Object[]> findProductTransactionLogsByProductNameAndBrand(@Param("productName") String productName,
                                                                   @Param("brand") String brand);
